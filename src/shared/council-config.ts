...
export function clampTimeoutSeconds(raw: number): number {
  ...
  return Math.min(
    MAX_TIMEOUT_SECONDS,
    Math.max(MIN_TIMEOUT_SECONDS, Math.round(raw)),
  );
}
...
export function validateCouncilConfig(config: CouncilConfig): string[] {
  ...
  if (typeof t !== "number" || !Number.isFinite(t)) {
    ...
    errors.push(
      `Council timeout must be between ${MIN_TIMEOUT_SECONDS} and ${MAX_TIMEOUT_SECONDS} seconds`,
    );
  }
...
export function resolveActiveMembers(
  ...
  const active = COUNCIL_AGENT_IDS.filter(
    (id) => config.members[id] && detected[id],
  );
  if (!config.enabled) {
    return { canRun: false, active, reason: "设置中的委员会规划已禁用。" };
  }
  if (active.length < MIN_COUNCIL_MEMBERS) {
    return {
      canRun: false,
      active,
      reason: `委员会至少需要 ${MIN_COUNCIL_MEMBERS} 个代理。请安装或启用 Pi、Claude 或 Codex，或关闭委员会规划。`,
    };
  }
...
function untrustedPlanSection(id: CouncilAgentId, plan: string): string {
  return formatUntrustedBlock(
    `PROPOSED PLAN FROM ${AGENT_LABELS[id]}`,
    plan,
    COUNCIL_PLAN_NOTE,
  );
}
 ...
 */
export function parseCodexStreamLine(line: string): {
  plan?: string;
  display?: string;
} {
  const trimmed = line.trim();
 ...
 */
export function parsePiStreamLine(line: string): {
  plan?: string;
  display?: string;
} {
  const trimmed = line.trim();
  ...
  const event = obj.assistantMessageEvent as
