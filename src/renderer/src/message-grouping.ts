...
export function toolLabel(name: string): string {
  ...
  if (
    ...
    return "Fetch URL";
  if (n.includes("edit") || n.includes("replace") || n.includes("patch"))
    return "Edit file";
...
const TOOL_VERBS: Record<string, ToolVerb> = {
  "Fetch URL": {
    ...
    noun: "URL",
    nounPlural: "URLs",
    argKeys: ["url", "uri", "href", "link"],
  },
  "Read file": {
    verb: "Read",
    noun: "file",
    nounPlural: "files",
    argKeys: ["path", "file", "filename", "file_path", "filepath"],
  },
  "Run command": {
    verb: "Ran",
    noun: "command",
  ...
  Search: {
    ...
    noun: "query",
    nounPlural: "queries",
    argKeys: ["query", "pattern", "text", "search", "q", "regex"],
  },
  "List files": {
    verb: "Listed",
    noun: "location",
    nounPlural: "locations",
    argKeys: ["path", "dir", "directory", "location", "folder"],
  },
};

// Fallback for custom/unknown tools so mixed or unknown runs still read sensibly.
const GENERIC_VERB: ToolVerb = {
  verb: "Ran",
  noun: "tool",
  nounPlural: "tools",
  argKeys: [],
...
const TOOL_DISPLAY: Record<
  string,
  { label: string; verb: string; noun: string; nounPlural: string }
> = {
  "Fetch URL": {
    label: "获取 URL",
    verb: "已获取",
  ...
  Search: { label: "搜索", verb: "已搜索", noun: "查询", nounPlural: "查询" },
  "List files": {
    label: "列出文件",
    verb: "已列出",
    noun: "位置",
...
export function localizedToolLabel(name: string): string {
  const label = toolLabel(name);
  return TOOL_DISPLAY[label]?.label ?? label;
}
...
/** Localized version used by the renderer; canonical labels stay English for logic. */
export function localizedToolCallLabel(
  ...
  const label = toolLabel(name);
  const v = TOOL_VERBS[label];
  const display = TOOL_DISPLAY[label];
  if (!v || !display) return label;
  const arg = extractArg(v, argumentsJson);
  return arg
    ? `${display.verb} ${displayArg(label, arg)}`
    : `${display.verb}${display.noun}`;
}
...
function groupTitle(run: DisplayMessage[]): string {
  ...
  return order
    .map((key, i) => {
      ...
      const verb =
        i === 0 ? v.verb : v.verb.charAt(0).toLowerCase() + v.verb.slice(1);
...
/** Translate a canonical group title at the final renderer boundary. */
export function localizedToolGroupTitle(title: string): string {
  return title
    .replace(/Fetched (\d+) URLs?/gi, "已获取 $1 个 URL")
    .replace(/Fetched a URL/gi, "已获取 1 个 URL")
    .replace(/Read (\d+) files?/gi, "已读取 $1 个文件")
    .replace(/Read a file/gi, "已读取 1 个文件")
    .replace(/Ran (\d+) commands?/gi, "已运行 $1 个命令")
    .replace(/Ran a command/gi, "已运行 1 个命令")
    .replace(/Edited (\d+) files?/gi, "已编辑 $1 个文件")
    .replace(/Edited a file/gi, "已编辑 1 个文件")
    .replace(/Created (\d+) files?/gi, "已创建 $1 个文件")
    .replace(/Created a file/gi, "已创建 1 个文件")
    .replace(/Searched (\d+) queries?/gi, "已搜索 $1 个查询")
    .replace(/Searched a query/gi, "已搜索 1 个查询")
    .replace(/Listed (\d+) locations?/gi, "已列出 $1 个位置")
    .replace(/Listed a location/gi, "已列出 1 个位置")
    .replace(/Ran (\d+) tools?/gi, "已运行 $1 个工具")
    .replace(/Ran a tool/gi, "已运行 1 个工具")
    .replace(/, /g, "，");
}
...
export function groupToolMessages(
  ...
  const flush = (): void => {
    ...
    const toolCallCount = run.reduce(
...
export function parseEdits(argumentsJson: string): EditBlock[] | null {
  ...
  if (Array.isArray(edits)) {
    ...
    for (const e of edits) {
      ...
      const oldText = stringField(block, [
        "oldText",
        "old_text",
        "old_string",
        "oldString",
      ...
      const newText = stringField(block, [
        "newText",
        "new_text",
        "new_string",
        "newString",
...
export function editStats(blocks: EditBlock[]): {
  added: number;
  removed: number;
} {
  let added = 0;
 ...
 */
export function splitReadTruncationNote(content: string): {
  code: string;
  note: string | null;
} {
  ...
  if (last < 0 || !READ_TRUNCATION_RE.test(lines[last].trim()))
    return { code: content, note: null };
  ...
  for (const m of messages) {
    if (m.role === "assistant" && m.toolCalls) {
      for (const tc of m.toolCalls) {
        calls.set(tc.id, {
          name: tc.name,
          file: toolCallFile(tc.name, tc.arguments),
        });
      }
    }
  }
