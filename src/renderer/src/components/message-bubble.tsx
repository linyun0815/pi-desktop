...
function MessageBubbleImpl({
  ...
  const handleBranch = () => {
    ...
    useAppStore.getState().addMessage({
      id: `branch-${Date.now()}`,
      role: "system",
      content: `已从以下消息创建分支：“${message.content.slice(0, 100)}${message.content.length > 100 ? "..." : ""}”`,
      timestamp: Date.now(),
    });
  };
  ...
  const { show: showContextMenu, ContextMenuComponent: MessageContextMenu } =
    useContextMenu();
  ...
  const handleMessageContextMenu = (e: React.MouseEvent) => {
    showContextMenu(
      e,
      buildMessageContextMenu(message.content, startNoteFromText),
    );
  };
  ...
  return (
    <div
      className={clsx(
        ...
        isGroupedPureThinking ? "-mt-2 mb-2" : "mb-4",
      )}
    >
      <div className="flex items-start gap-3">
...
function ToolCallBadge({
  ...
  const edits =
    toolLabel(toolCall.name) === "Edit file"
      ? parseEdits(toolCall.arguments)
      : null;
  const stats = edits ? editStats(edits) : null;
  const editFile = edits
    ? toolCallFile(toolCall.name, toolCall.arguments)
    : null;
  const editLang = editFile
    ? getCodeEditorLanguageName(editFile)
    : "plain text";
  ...
  return (
    <div className="relative min-w-0 overflow-hidden rounded-lg border border-border bg-surface/50">
      <CopyButton
        text={toolCallCopyText(toolCall)}
      ...
      >
        <span className="font-jetbrains min-w-0 truncate">
          {localizedToolCallLabel(toolCall.name, toolCall.arguments)}
        </span>
        {stats && (
...
// React renders the highlighter's text segments directly, so source content
// never enters an HTML string or a dangerouslySetInnerHTML sink.
function DiffLines({
  text,
  lang,
  kind,
  ...
  if (text === "") return null;
  const highlighted = highlightCodeToSegments(text, lang);
  const highlightedLines = highlighted
    ? splitHighlightSegments(highlighted)
    : null;
  const plainLines = highlightedLines ? null : text.split("\n");
  ...
  return (
    <>
      {Array.from({ length: lineCount }, (_unused, j) => (
        <div key={j} className={clsx("whitespace-pre", rowBg)}>
          <span className={clsx("select-none", markColor)}>{sign}</span>
          {highlightedLines ? (
            <span>{renderHighlightedLine(highlightedLines[j] ?? [])}</span>
          ) : (
            <span>{plainLines?.[j] || " "}</span>
          )}
        </div>
      ))}
    </>
...
function ToolResultMessage({
  ...
  const firstLine =
    newlineIdx === -1 ? message.content : message.content.slice(0, newlineIdx);
...
function CodeResultView({
  ...
  const clippedContent = clipped
    ? content.slice(0, MAX_CODE_RESULT_CHARS)
    : content;
        ...
        <div className="min-w-0 flex-1">
          {showSharedHeader && (
            <div className="flex h-7 items-center gap-2 text-sm text-dim">
              ...
              <span className="text-ghost">·</span>
              <span>
                {modelDisplayName(sharedModel as string, customModels)}
              </span>
...
function ActionButton({
  ...
  return (
    <button
      ...
      title={localizedToolGroupTitle(title)}
      aria-label={localizedToolGroupTitle(title)}
    >
      {icon}
