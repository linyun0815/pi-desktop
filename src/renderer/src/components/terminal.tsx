...
export function TerminalPanel(): React.JSX.Element | null {
  ...
  const [shellLabel, setShellLabel] = useState<string>("终端");
  ...
  useEffect(() => {
    ...
    const terminal = new XTerm({
      ...
      convertEol: true,
      fontFamily:
        "'JetBrains Mono Variable', 'JetBrains Mono', 'Fira Code', 'Cascadia Code', monospace",
    ...
    window.setTimeout(async () => {
      ...
      try {
        ...
        terminal.writeln(`启动终端失败：${formatUiError(err)}`);
      }
      terminal.focus();
    }, 0);
