  ...
  return (
    <div className="my-2 rounded-lg border border-border-strong bg-surface/60 p-3">
      ...
      >
        ...
        <span>委员会规划 · {PHASE_LABEL[run.phase] ?? run.phase}</span>
        {consulting && startedAt ? (
          <span className="text-dim">({formatElapsed(elapsed)})</span>
        ) : null}
        {collapsed && run.results.length > 0 ? (
          <span className="ml-2 normal-case text-dim">
            {run.results
              .map(
                (r) =>
                  `${AGENT_LABEL[r.id] ?? r.id} ${r.status === "contributed" ? "✓" : "✕"}`,
              )
              .join(" · ")}
          </span>
        ) : null}
      ...
      {!collapsed && (
        <div className="mt-2">
          {run.phase === "refused" && (
            <div className="text-sm text-warning">{run.reason}</div>
          )}
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {run.members!.map((id) => {
                ...
                return (
                  <div
                    key={id}
                    className="rounded border border-border bg-app p-2"
                  >
      ...
      {(run.phase === "merging" || awaiting) && (
        <div className="mt-2 rounded border border-border bg-app p-2">
          <div className="mb-1 flex items-center justify-between">
            <span className="text-sm text-primary">共识方案</span>
            <span className="text-xs text-accent-fg">
              {run.phase === "merging"
                ? run.consensus
                  ? "合并中…"
