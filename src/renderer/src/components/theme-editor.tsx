...
import {
  ...
  withSeed,
  withSyntax,
} from "./theme-editor-helpers";
import { localizedTokenLabel } from "../utils/ui-text";
...
const ADVANCED_TOKENS = TOKEN_NAMES.filter(
  (token) => !SEED_BACKED_TOKENS.has(token),
);
...
function tokenLabel(name: string): string {
  return localizedTokenLabel(name);
}
...
function resolveSettingsThemeId(
...
export function ThemeEditor({
  ...
  const [draft, setDraft] = useState<ThemeFile>(() =>
    isUserTheme
      ...
      : forkTheme(baseTheme, `${baseTheme.name} 副本`),
  );
  ...
  const preview = useCallback((next: ThemeFile) => {
    ...
    for (const token of TOKEN_NAMES)
      read[token] = style.getPropertyValue(cssVarForToken(token)).trim();
    for (const key of SYNTAX_KEYS)
      read[`cm:${key}`] = style.getPropertyValue(`--cm-${key}`).trim();
    setEffective(read);
  }, []);
  ...
  const save = async () => {
    ...
    try {
      ...
      const { id } = await window.piDesktop.themes.save(
        payload,
        isUserTheme ? baseId : undefined,
      );
      ...
      if (isUserTheme && id !== baseId) {
        try {
          ...
          warning = `主题已保存为“${draft.name}”，但旧版本无法删除，可能仍会出现在列表中。`;
        }
      }
      onSaved(id, warning);
    } catch (error) {
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              ...
              <input
                ...
                onChange={(event) =>
                  preview({ ...draft, name: event.target.value })
                }
            ...
            <div>
              <span className="mb-1 block text-xs text-dim">类型</span>
          ...
          <div className="flex items-start gap-4">
            <div className="w-56">
              ...
              <input
                ...
                placeholder="显示在主题库中"
                onChange={(event) =>
                  preview({ ...draft, author: event.target.value })
                }
                className="w-full rounded-md border border-border-strong bg-surface px-3 py-1.5 text-sm text-primary focus:border-focus focus:outline-none"
              />
            </div>
            <div className="flex-1">
              <label
                className="mb-1 block text-xs text-dim"
                htmlFor="theme-editor-description"
              >
                描述（可选）
              </label>
              <input
                ...
                placeholder="用一两句话介绍主题"
                onChange={(event) =>
                  preview({ ...draft, description: event.target.value })
                }
                className="w-full rounded-md border border-border-strong bg-surface px-3 py-1.5 text-sm text-primary focus:border-focus focus:outline-none"
              />
            </div>
            ...
            {SEED_NAMES.map((seed) => (
              ...
              >
                <label
                  ...
                  htmlFor={`theme-editor-seed-${seed}`}
                >
                  {SEED_LABELS[seed]}
                </label>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(event) =>
                      preview(withSeed(draft, seed, event.target.value))
                    }
              ...
              {ADVANCED_TOKENS.map((token) => {
                ...
                return (
                  <TokenPinRow
                    ...
                    onPin={(value) =>
                      preview(withOverride(draft, token, value))
                    }
