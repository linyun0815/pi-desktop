...
interface StatsStoreDeps {
  /** Legacy single-root override retained for focused tests and callers. */
  sessionsRoot?: string;
  /** Read multiple engine stores in production or in cross-engine tests. */
...
function parseSessionContent(content: string): Record<string, DayBucket> {
  ...
  for (const line of content.split("\n")) {
    ...
    if (rec.type !== MESSAGE_RECORD_TYPE || typeof rec.timestamp !== "string")
      continue;
...
export class ActivityStatsStore {
  ...
  private readonly sessionsRootsOverride?: string[];
  ...
  constructor(deps: StatsStoreDeps = {}) {
    ...
    this.sessionsRootsOverride = deps.sessionsRoots;
  ...
  private roots(): string[] {
    const configured =
      this.sessionsRootsOverride ??
      (this.sessionsRoot ? [this.sessionsRoot] : getSessionRoots());
    return [...new Set(configured.map((root) => resolve(root)))];
  }

  private modelsPaths(): string[] {
    ...
    try {
      if (existsSync(path)) {
        ...
        if (
          ...
          for (const entry of Object.values(p.sessions)) {
            if (
              !entry ||
              typeof entry !== "object" ||
          ...
          this.store = {
            ...
            sessions,
            modelNames:
              p.modelNames && typeof p.modelNames === "object"
                ? p.modelNames
                : {},
          };
        }
  ...
  private updateModelNames(): boolean {
    ...
    for (const modelsPath of this.modelsPaths()) {
      try {
        ...
        // Missing or unreadable models.json: keep whatever names we have.
      }
    }
    return changed;
  }
  ...
  captureBeforeDelete(sessionPath: string): void {
    this.ensureLoaded();
    const id = sessionStoreKey(sessionPath);
    try {
