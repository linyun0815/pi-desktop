...
export class PiSdkManager extends EventEmitter {
  ...
  private settleInit: ((result: { ok: boolean; error?: string }) => void) | null = null
  ...
  private async doStart(options: PiStartOptions): Promise<PiStatus> {
    ...
    const initOutcome = await new Promise<{ ok: boolean; error?: string }>((resolve) => {
      const timer = setTimeout(
        () => resolve({ ok: false, error: `helper init timed out after ${INIT_TIMEOUT_MS}ms` }),
        INIT_TIMEOUT_MS,
      )
      timer.unref?.()
    ...
    if (this.status === 'starting') {
      console.log('[PiSDK] Helper ready:', this.child?.pid, 'session:', this.sessionPath)
  ...
  private buildInitMessage(options: PiStartOptions): HelperInitMessage {
    ...
    return {
      ...
      ...(options.permissionRulesPath ? { permissionRulesPath: options.permissionRulesPath } : {}),
  ...
  private handleHelperMessage(value: unknown): void {
    ...
    switch (msg.kind) {
      ...
      case 'uiRequest':
        ...
        {
          const event = { ...msg.request, type: 'extension_ui_request' } as unknown as PiRpcEvent
  ...
  private handleExit(code: number | undefined): void {
    ...
    this.settleInit?.({ ok: false, error: `helper exited with code ${code ?? 'null'} during init` })
    this.sessionBoundPending?.()
  ...
  async request(
    ...
    return new Promise<PiResponseEvent>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pendingResponses.delete(id)
        reject(new Error(`Command ${command} timed out after ${this.timeoutFor(command)}ms`))
  ...
  async prompt(
    ...
    return this.request(
      { kind: 'prompt', message, ...(options?.images ? { images: options.images } : {}), ...(options?.streamingBehavior ? { streamingBehavior: options.streamingBehavior } : {}) },
      'prompt',
    )
  }

  async steer(message: string, images?: PromptImage[]): Promise<PiResponseEvent> {
    return this.request(
  ...
  async setThinkingLevel(level: ProtocolThinkingLevel): Promise<PiResponseEvent> {
    return this.request({ kind: 'setThinkingLevel', level }, 'set_thinking_level')
  }
  ...
  async sessionSwitch(sessionPath: string): Promise<PiResponseEvent> {
    return this.request({ kind: 'sessionSwitch', sessionPath }, 'switch_session')
  }

  async sessionFork(entryId?: string): Promise<PiResponseEvent> {
    return this.request({ kind: 'sessionFork', ...(entryId ? { entryId } : {}) }, 'fork')
  }
  ...
  async compact(customInstructions?: string): Promise<PiResponseEvent> {
    return this.request(
      { kind: 'compact', ...(customInstructions ? { customInstructions } : {}) },
      'compact',
    )
  }
   ...
   */
  sendExtensionUiResponse(responseId: string, response: Record<string, unknown>): void {
