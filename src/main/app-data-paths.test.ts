...
test("getExternalGuiDataDir returns the externally-set override as an absolute path", () => {
  ...
  assert.equal(
    ...
    resolve("/tmp/pi-desktop-scratch"),
  );
  assert.equal(
...
test("getGuiDataPath / getLegacyGuiDataPath resolve under the right root", () => {
  assert.equal(
    getGuiDataPath("settings.json", {
      userDataDir: "/home/test/.config/pi-desktop",
    }),
    join("/home/test/.config/pi-desktop", "settings.json"),
  );
  assert.equal(
    getLegacyGuiDataPath("settings.json", { homeDir: "/home/test" }),
    join("/home/test", ".pi-desktop-gui", "settings.json"),
  );
  const userDataDir = "/home/test/.config/pi-desktop";
  for (const fileName of GUI_DATA_FILES) {
    assert.equal(
      getGuiDataPath(fileName, { userDataDir }),
      join(userDataDir, fileName),
    );
  }
});
