...
import type { ProviderAuthInfo } from '../shared/ipc-contracts'
 ...
 *
 * The admin helper is a lazy utility process that performs everything
 * credential- and package-related against the SDK — API-key login/logout,
 * provider status, package install/remove/update, npm availability — so
 * extension/SDK code never loads into the Electron main process. There is at
 * most one admin helper per app run; it starts on first use and exits only
 * with the app or an explicit shutdown.
 *
 * Auth prompts flow through `onAuthPrompt` (typically a broadcast to the
 * renderer); the renderer's answers return via `answerAuthPrompt`, and a flow
 * is cancelled with `cancelLogin`. Secrets traverse exactly one message hop
 * into the helper and are never logged here.
 */

const ADMIN_INIT_TIMEOUT_MS = 30_000
const DEFAULT_ADMIN_TIMEOUT_MS = 60_000
/** Login waits on a human typing a key into a modal — give it room. */
...
export interface AdminDeps {
  /** Forward one interactive auth prompt (renderer modal). */
  onAuthPrompt(loginId: string, prompt: AuthPromptPayload): void
  /** Forward one non-interactive auth progress/info event. */
  onAuthNotify(loginId: string, event: AuthEventPayload): void
  cwd(): string
}
...
export class EmbeddedPiAdminManager {
  ...
  private async doStart(): Promise<void> {
    ...
    child.on('exit', (code) => {
      ...
      this.pending.clear()
      if (code !== 0) appLog.warn('pi-admin', `helper exited with code ${code ?? 'null'}`)
    })
    this.child = child
    await new Promise<void>((resolve, reject) => {
      const id = 'admin-init'
      const timer = setTimeout(() => reject(new Error('admin helper init timed out')), ADMIN_INIT_TIMEOUT_MS)
      this.pending.set(id, {
        resolve: () => {
          clearTimeout(timer)
          resolve()
      ...
      this.send({ kind: 'admin-init', protocolVersion: EMBEDDED_AGENT_PROTOCOL_VERSION, agentDir: getPiAgentDir(), cwd: this.deps.cwd() }, id)
    })
  }
  ...
  private handleMessage(value: unknown): void {
    ...
    switch (msg.kind) {
      case 'adminResponse': {
        ...
        else pending.reject(new Error(msg.error ?? `command ${msg.command} failed`))
        return
      }
      case 'authPrompt':
        this.deps.onAuthPrompt(msg.loginId, msg.prompt)
        return
      case 'authNotify':
        this.deps.onAuthNotify(msg.loginId, msg.event)
        return
      case 'adminPackageProgress':
        // Progress rides the output string on completion; keep logs light.
        appLog.info('pi-admin', `[package ${msg.event.action}] ${msg.event.message ?? ''}`)
        return
      case 'log':
  ...
  private send(message: object, correlationId?: string): void {
  ...
  async listProviders(): Promise<ProviderAuthInfo[]> {
    ...
    const data = (await this.request({ kind: 'adminListProviders' }, 'list_providers')) as
  ...
  /**
   * Run the SDK api_key login for one provider. Prompts raised by the SDK are
   * forwarded through deps (already tagged with `loginId`); the renderer's
   * answers return via `answerAuthPrompt`, cancellation via `cancelLogin`.
   */
  async login(providerId: string, loginId: string): Promise<void> {
    await this.ensureStarted()
    try {
      await this.request(
        { kind: 'adminLogin', loginId, providerId },
        'login',
        LOGIN_TIMEOUT_MS,
      )
    } finally {
      // Prompts left hanging (a dismissed modal) die with the request reject.
    }
  }

  /** Deliver the renderer's answer to one forwarded auth prompt. */
  answerAuthPrompt(loginId: string, answer: { value?: string; canceled?: boolean }): void {
    try {
      this.send({
        kind: 'adminPromptAnswer',
        loginId,
        ...(answer.value !== undefined ? { value: answer.value } : {}),
        ...(answer.canceled ? { canceled: true } : {}),
      })
    } catch {
      appLog.warn('pi-admin', 'cannot answer auth prompt: helper is not running')
    }
  }
  ...
  async cancelLogin(loginId: string): Promise<void> {
    ...
    try {
      this.send({ kind: 'adminCancelLogin', loginId })
    } catch {
      // Helper already gone.
    }
  }
  ...
  async logout(providerId: string): Promise<void> {
    await this.ensureStarted()
    await this.request({ kind: 'adminLogout', providerId }, 'logout')
  }
  ...
  async installPackage(source: string): Promise<AdminPackageResult> {
    await this.ensureStarted()
    try {
      await this.request({ kind: 'adminPackageInstall', source }, 'package_install', 300_000)
  ...
  async removePackage(source: string): Promise<AdminPackageResult> {
    await this.ensureStarted()
    try {
      await this.request({ kind: 'adminPackageRemove', source }, 'package_remove', 120_000)
  ...
  async updatePackage(source?: string): Promise<AdminPackageResult> {
    await this.ensureStarted()
    try {
      await this.request({ kind: 'adminPackageUpdate', ...(source ? { source } : {}) }, 'package_update', 300_000)
      return { success: true, output: source ? `已更新 ${source}` : '已更新全部包' }
    } catch (err) {
      return { success: false, output: err instanceof Error ? err.message : String(err) }
    }
  }
  ...
  async toolAvailability(): Promise<{ npm: boolean; git: boolean }> {
    await this.ensureStarted()
    const data = (await this.request({ kind: 'adminNpmAvailable' }, 'tool_availability')) as
