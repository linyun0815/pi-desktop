...
test("saving identical id and name overwrites (editor update path)", async () => {
  ...
  await saveUserTheme(dir, {
    ...theme("Same"),
    seeds: { ...theme("Same").seeds, app: "#000000" },
  });
  const { themes } = await listUserThemes(dir);
...
test("editing a theme onto another theme's name suffixes instead of overwriting", async () => {
  ...
  const barContent = {
    ...theme("Bar"),
    seeds: { ...theme("Bar").seeds, app: "#abcdef" },
  };
...
test("editing a theme without renaming overwrites its own file in place", async () => {
  ...
  const edited = {
    ...theme("Bar"),
    seeds: { ...theme("Bar").seeds, app: "#000000" },
  };
...
test("existingId must be a valid theme id", async () => {
  ...
  await assert.rejects(
    saveUserTheme(dir, theme("Foo"), "../escape"),
    /invalid theme id/,
  );
});
...
test("existingId cannot target a built-in theme id", async () => {
  const dir = await freshDir();
  await assert.rejects(saveUserTheme(dir, theme("Foo"), "nord"), /built-in/);
});
...
test("list ignores a file whose id shadows a built-in theme", async () => {
  ...
  await writeFile(
    join(dir, "nord.json"),
    JSON.stringify(theme("Not Actually Nord")),
  );
  const { themes, warnings } = await listUserThemes(dir);
...
test("installThemeFromUrl validates scheme, size, and content", async () => {
  ...
  const { id } = await installThemeFromUrl(
    dir,
  ...
  assert.ok(
    (await readFile(join(dir, "remote.json"), "utf8")).includes("Remote"),
  );

  await assert.rejects(
    installThemeFromUrl(dir, "http://example.com/t.json", okFetch),
    /https/,
  );
  const bigFetch = (async () =>
    new Response("x".repeat(300000), { status: 200 })) as typeof fetch;
  await assert.rejects(
    installThemeFromUrl(dir, "https://example.com/big.json", bigFetch),
    /too large/,
  );
  const badFetch = (async () =>
    new Response('{"$schema":"nope"}', { status: 200 })) as typeof fetch;
  await assert.rejects(
    installThemeFromUrl(dir, "https://example.com/bad.json", badFetch),
    /ThemeValidationError|unsupported/,
  );
  const failFetch = (async () =>
    new Response("", { status: 404 })) as typeof fetch;
  await assert.rejects(
    installThemeFromUrl(dir, "https://example.com/404.json", failFetch),
    /404/,
  );
});
...
function trackingFetch(responses: Record<string, () => Response>): {
...
for (const url of PRIVATE_IPV4_URLS) {
  test(`installThemeFromUrl rejects private IPv4 literal ${url}`, async () => {
    ...
    await assert.rejects(
      installThemeFromUrl(dir, url, fetchFn),
      /block|private|internal|reserved/i,
    );
...
test("installThemeFromUrl blocks a public URL that redirects to an internal host", async () => {
  ...
  const { fetchFn, calls } = trackingFetch({
    [publicUrl]: () =>
      new Response(null, { status: 302, headers: { location: internalUrl } }),
  });
  await assert.rejects(
    installThemeFromUrl(dir, publicUrl, fetchFn),
    /block|private|internal|reserved/i,
  );
...
test("installThemeFromUrl throws when redirects exceed the cap", async () => {
  ...
  for (let i = 0; i < hopCount; i += 1) {
    responses[`${base}${i}`] = () =>
      new Response(null, {
        status: 302,
        headers: { location: `${base}${i + 1}` },
      });
  }
  const { fetchFn } = trackingFetch(responses);
  await assert.rejects(
    installThemeFromUrl(dir, `${base}0`, fetchFn),
    /redirect/i,
  );
});
...
test("installThemeFromUrl still installs from a normal public https URL", async () => {
  ...
  const { fetchFn, calls } = trackingFetch({
    "https://public.example.com/plain.json": () =>
...
test("installThemeFromUrl rejects a redirect that downgrades to http", async () => {
  ...
  const { fetchFn, calls } = trackingFetch({
    [httpsUrl]: () =>
      new Response(null, { status: 302, headers: { location: httpUrl } }),
  });
...
test("fetchGalleryThemes returns valid entries with pinned first-party URLs", async () => {
  ...
  assert.deepEqual(
    themes.map(({ name, kind, url }) => ({ name, kind, url })),
    [
      {
        name: "Ocean",
        kind: "dark",
        url: "https://raw.githubusercontent.com/FaqFirebase/pi-desktop-themes/main/themes/ocean/theme.json",
      },
      {
        name: "Ember Light",
        kind: "light",
        url: "https://raw.githubusercontent.com/FaqFirebase/pi-desktop-themes/main/themes/ember-light.json",
      },
    ],
  );
});
...
test("fetchGalleryThemes drops malformed and path-traversal entries", async () => {
  const themes = await fetchGalleryThemes(
    jsonFetch([
      ...
      {
        name: "Folder traversal",
        kind: "dark",
        file: "themes/x/../y/theme.json",
      },
      ...
      { name: "Wrong file name", kind: "dark", file: "themes/x/other.json" },
      {
        name: "Absolute",
        kind: "dark",
        file: "https://evil.example.com/x.json",
      },
...
test("fetchGalleryThemes rejects a non-array index and a failed fetch", async () => {
  await assert.rejects(
    fetchGalleryThemes(jsonFetch({ not: "an array" })),
    /not a JSON array/,
  );
  const failFetch = (async () =>
    new Response("", { status: 500 })) as typeof fetch;
  await assert.rejects(
    fetchGalleryThemes(failFetch),
    /gallery index download failed: 500/,
  );
});
    ...
    jsonFetch([
      {
        name: "Shot",
        kind: "dark",
        file: "themes/shot/theme.json",
        screenshot: "themes/shot/screenshot.png",
      },
      {
        name: "Bad",
        kind: "dark",
        file: "themes/bad/theme.json",
        screenshot: "themes/bad/../evil.png",
      },
      { name: "None", kind: "dark", file: "themes/none/theme.json" },
    ]),
  );
  assert.equal(
    themes[0].screenshotUrl,
    `${GALLERY_BASE}/themes/shot/screenshot.png`,
  );
...
test("fetchGalleryImage pins the URL and requires an image response", async () => {
  ...
  await assert.rejects(
    fetchGalleryImage("https://evil.example.com/x.png", imgFetch),
    /not an allowed gallery path/,
  );
  // Right host, but not a screenshot path.
  await assert.rejects(
    fetchGalleryImage(`${GALLERY_BASE}/themes/shot/theme.json`, imgFetch),
    /not an allowed gallery path/,
  );
  ...
  await assert.rejects(
    fetchGalleryImage(good, htmlFetch),
  ...
  await assert.rejects(
    fetchGalleryImage(good, svgFetch),
    /not an allowed image type/,
  );
...
test("fetchGalleryImage caps oversized screenshots", async () => {
  ...
  await assert.rejects(
    fetchGalleryImage(`${GALLERY_BASE}/themes/shot/screenshot.png`, hugeFetch),
    /too large/,
  );
});
