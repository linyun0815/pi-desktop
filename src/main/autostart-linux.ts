...
const APP_COMMENT = "登录时自动启动 Pi Desktop";
...
export function linuxAutostartPath(
   ...
   const configHome =
      env.XDG_CONFIG_HOME?.trim() || posixPath.join(home, ".config");
 ...
 */
export function linuxLaunchExec(
   env: NodeJS.ProcessEnv = process.env,
...
export function buildLinuxDesktopEntry(opts: {
   exec: string;
   icon: string;
}): string {
   return [
