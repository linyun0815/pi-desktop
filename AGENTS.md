...
│   ├── models-config.ts          # Custom models.json validate/merge (cost rates + tiers, model usage keys)
│   ├── model-catalog.ts          # models.dev catalog parsing/matching (pure; fetch+cache in main)
│   ├── model-discovery.ts        # Provider /models discovery helpers (endpoint build, response parse, redirect guard)
...
│   ├── activity-stats.ts         # Persisted per-day message/token/model stats store (four token kinds + cost, v2)
│   ├── package-catalog.ts        # pi.dev catalog crawl, concurrent + prefetched + cached
│   ├── model-catalog.ts          # models.dev catalog fetch + 1h cache + shared in-flight lookup
...
- Model metadata auto-fill and provider discovery: the editor can fill a model's name/reasoning/modalities/windows/prices from the models.dev catalog (`shared/model-catalog.ts`, fetched once by main and cached for an hour; exact provider → base-URL host → consensus matching, prices only injected when reliable), and can list a provider's models via its own `/models` endpoint (see the Embedded Pi runtime section). Suggestions only fill empty draft fields — explicit `false`/`0`/existing values and unknown fields are never overwritten — and are written to models.json only by the explicit Save
...
- Range-selectable (7d–1y) stats: sessions, messages, tokens, active days, current/longest streak, peak hour, total cost, and per-model input/output/cache-read/cache-write usage. **Token totals are the four-kind sum** (`input + output + cacheRead + cacheWrite`; `cacheWrite1h` is a subset of cacheWrite, reasoning tokens live inside output) and historical cost comes from each assistant message's recorded `usage.cost.total` — never re-estimated from current model prices
...
- Custom models & providers editor — edits `~/.pi/agent/models.json`. Providers render as collapsible cards; each model has a three-state `thinkingLevelMap` editor per level (default mapping / unsupported (`null`) / custom provider value) in `custom-models-editor.tsx` + `custom-models-editor-helpers.ts` (pure conversions/validation, unit-tested). `apiKey` inputs are password-masked with a show/hide toggle and are never echoed in logs, status, or errors. Each model also has four price inputs (USD per million tokens; zero is a legal free price, clearing all four removes `cost`), a catalog auto-fill button per model, and a provider discovery button with a searchable model list. Saving validates (empty/duplicate keys, empty custom values, invalid cost values), overlays the edited fields while preserving unknown JSON fields (`mergeModelsConfig`; an editor-supplied map replaces the original outright, an empty map removes it), and main hot-reloads idle runtimes' ModelRuntime during the write — busy runtimes pick the new config up on their next start, and the editor says which state applies
